...
export const content = {
  ko: {
    ...
    nav: { engine: '문서 검증 엔진', rtm: '감리 대비표 사전 검토', miriboa: '입찰 서류 검증', data: '데이터 피드', search: '검색 품질 진단', notes: '실측 노트', work: '만든 것들', about: '회사 소개', contact: '문의', founder: '대표 소개' },
    ...
    home: {
      ...
      title: '시즐론 | 문서 검증 엔진·데이터 파이프라인·한국어 검색',
      description: '시즐론은 소프트웨어 회사입니다. 한국 공공 문서를 표째 읽고 대조하는 문서 검증 엔진, 매일 도는 데이터 파이프라인, 한국어 검색을 만들고 직접 운영합니다. 감리 대비표 사전 검토·입찰 서류 검증·데이터 피드·검색 품질 진단. 어떻게 재는지와 얼마나 틀리는지를 공개합니다.',
      // 09-14 오너: 옛 h1("읽고·모으고·찾게 만드는")이 모호 → 동사를 시스템 셋과 1:1 로. 쉼표마다 한 줄(h1Lines 로 렌더).
      h1: '문서를 대조하는 엔진을 만들고, 공공 데이터를 수집·정제하고, 한국어 검색 시스템을 고칩니다.',
      h1Lines: ['문서를 대조하는 엔진을 만들고,', '공공 데이터를 수집·정제하고,', '한국어 검색 시스템을 고칩니다.'],
      lede: '시즐론은 소프트웨어 회사입니다. 문서 검증 엔진과 데이터 파이프라인, 한국어 검색을 직접 만들고 운영하며, 그 위에서 서비스를 합니다. 수치는 잣대와 함께 공개하고, 잰 적 없는 것은 없다고 씁니다.',
      methodEyebrow: '작업 방식',
      method,
      systemsEyebrow: '만든 것',
      ...
      systems: [
        {
          key: 'engine', no: '01', name: '문서 검증 엔진', featured: true, href: '/engine/', hrefLabel: '엔진이 하는 일과 하지 않는 일',
    ...
    services: {
      ...
      rtm: {
        ...
        eyebrow: '서비스 · 감리 앞',
        crumb: { href: '/engine/', label: '문서 검증 엔진', here: '감리 앞' },
        ...
        who: [
          ...
          '요구사항추적표는 있는데 설계·시험 열이 비어 있는 팀, 지난 감리에서 대비표·추적성으로 개선권고를 받은 적이 있는 팀. 빠진 항목은 그대로 지적사항이 되고, 종료 감리를 못 넘기면 검수와 잔금이 밀립니다. 감리비는 발주기관이 내지만, 지적을 고치는 비용은 수행사 몫입니다.',
        ],
        howH: '진행',
        how: [
          { name: '자료 접수 — 당일', text: '꼭 필요한 것은 셋입니다. ① RFP(제안요청서) — 요구사항 고유번호 표가 들어 있는 원본 파일. ② 요구사항추적표 — 요구사항 ID 가 RFP 고유번호이거나 그 번호로 시작해야 하고(SFR-001, SFR-001-01), 자체 번호를 쓰면 RFP 고유번호를 적은 출처 열이 있어야 합니다. xlsx 로 주시면 그 파일 위에 결과를 붙입니다. ③ 추적표가 가리키는 ID(화면·프로그램 등)가 들어 있는 설계서 전부. 있으면 검사기준서도 함께 봅니다. NDA 뒤 파일로 받고, 하루 안에 범위와 고정가를 회신합니다.' },
          { name: '추출', text: 'RFP 의 요구사항 표를 규칙으로 읽어 고유번호(SFR-001 같은)마다 세부 항목을 뽑습니다. 18.5억 규모 실제 RFP 한 건에서 고유번호 54건이 나왔습니다.' },
          { name: '대조 — 세 겹', text: '감리 수행 가이드가 정한 감리원의 대비표 점검 세 겹(형식 요건·추적성·설계 반영)을 문서만으로 먼저 봅니다. 항목별로 무엇을 보고 무엇을 보지 않는지는 아래 "하는 것과 하지 않는 것"에 있습니다.', note: '세 겹의 출처와 감리원이 실제로 지적하는 것:', link: { href: '/notes/audit-findings/', label: '감리 지적 통계와 점검 절차' } },
          { name: '보고 — 영업일 5일 안', text: '반영·부분 반영·미반영 목록과 근거, 표에 판정을 붙인 검토본, 감리원 문장 틀로 쓴 지적 예상 목록을 드립니다. 엔진 출력 그대로이며 사람이 판정을 고치지 않습니다. 보완본을 주시면 재대조 1회는 가격에 포함됩니다.' },
        ],
        ...
        heroProof: [
          ...
          '감리 산출물 기준 정확도 측정치는 아직 없습니다. 고객이 보내 준 실제 감리 지적과 지적 예상 목록을 요구사항 ID로 대조한 수치를 쌓아 공개합니다.',
        ],
        ...
        scopeDo: {
          ...
          groups: [
            ...
            { label: '언어 모델 — 예상', items: [
              '설계 반영: 표가 가리킨 설계 절이 요구사항의 핵심 조건(수치·기한·권한·예외)을 담는지. 원문 인용을 붙여 적합 예상 · 부적합 예상 · 확인 필요로 표시합니다.',
            ] },
          ],
        },
        scopeDont: {
          ...
          items: [
            ...
            '설계서 사이의 일관성(화면·프로그램·테이블·인터페이스 설계서 간)',
            '설계 품질 자체와 다른 감리 영역(사업관리·데이터베이스·시스템 구조·보안)',
            '규칙 검사로 ID 내용 확인 — 규칙은 ID 가 문서에 나오는지만 봅니다(목차에만 있어도 "있음"). 내용은 언어 모델 판정이 봅니다',
            '요구정의 단계와 종료 단계 감리',
            '추적표 새로 만들기, 산출물 대신 써 주기',
        ...
        priceTable: {
          ...
          notes: [
            ...
            '보완본 재대조 1회 포함, 추가 청구 없음.',
            '사업비 20억 미만이거나 기간 6개월 미만인 사업은 감리기준상 2단계 감리(설계·종료)라 대개 설계단계 감리 1회가 이 가격표의 대상입니다. 20억 이상 3단계 감리의 요구정의 단계는 맡지 않습니다.',
          ],
        },
        ...
        faq: [
          ...
          { q: 'PMO 가 이미 추적표를 검토하는데요?', a: '표가 채워졌는지에 더해, 표가 가리킨 절을 열어 요구사항의 핵심 조건이 들어 있는지까지 봅니다. 2012년 감리보고서 169건 분석에서 지적 1순위가 "요구분석과 설계의 일관성"이었습니다.' },
          { q: '설계 산출물이 수백 쪽인데 다 읽나요?', a: '요구사항마다 표가 가리킨 절과 관련 절만 잘라 대조합니다. 문서 전체를 요약하지 않고, 근거는 항상 그 절의 원문 인용입니다.' },
          { q: '결과가 틀리면 어떻게 되나요?', a: '판정마다 원문 인용이 붙어 있어 PM 이 맞는지 바로 확인할 수 있습니다. 사람이 중간에 판정을 고치지 않으므로 틀린 판정은 인용을 보고 걸러 주셔야 하고, 표결이 갈리거나 인용이 원문에서 확인되지 않는 항목은 판정하지 않고 "확인 필요"로 둡니다. 감리 산출물 기준 정확도 측정치는 아직 없습니다. 고객이 보내 준 실제 감리 지적과 지적 예상 목록을 요구사항 ID로 대조한 수치를 쌓아 공개합니다.' },
          ...
          { q: '왜 이 가격인가요?', a: '대조할 요구사항 수로 정합니다. 2026년 1~5월 나라장터 제안요청서 중 예산 5억~20억 개발형 사업 87건의 요구사항 고유번호는 중앙값 40건이라, 절반이 가장 낮은 구간에 듭니다. 같은 규모 사업의 감리 계약 39건(나라장터 계약정보, 2026년 9월 수집)은 감리 계약금액 중앙값이 약 7,200만원(설계·종료 두 단계 합계)이고, 120~300만원은 그 2~4% 수준입니다. 정확도 측정치가 아직 없어, 감리가 끝난 뒤 지적 결과를 보내 측정에 협조해 주시면 일부를 돌려드립니다. 종료 감리를 통과하지 못해 검수와 잔금이 한 달 밀리는 비용과 비교하시면 됩니다. 범위 확정 뒤 고정가라 추가 청구가 없습니다.' },
          { q: '자료는 어떻게 다루나요?', a: 'NDA 체결 뒤 파일로 받고, 납품 후 15일 안에 파기합니다. 환급 대조를 위해 요구사항 ID와 판정만 담은 표(원문·인용 없음)는 대조가 끝날 때까지 남깁니다. 외부 인력 없이 대표가 직접 수행합니다. 설계 반영 판정에서는 요구사항마다 잘라 낸 설계 절만 언어 모델(Anthropic Claude, 학습 거부 설정)로 보내고, 문서 전체는 보내지 않습니다. 다른 고객의 문서는 어떤 결과물에도 쓰이지 않습니다.' },
        ],
        nextH,
        next: [
    ...
    bidcheck: {
      ...
      eyebrow: '서비스 · 입찰 마감 앞',
      crumb: { href: '/engine/', label: '문서 검증 엔진', here: '입찰 마감 앞' },
      h1: '입찰 서류 검증 — 미리보아',
    ...
    // ── 회사 소개 (v3 §3.6) ────────────────────────────────────────────
    // 문서 검증 엔진 페이지(2026-09-11 오너 결정: "포장하되 없는 것을 있다고 하지 않는다").
  ...
  en: {
    ...
    nav: { engine: 'Document verification engine', rtm: 'Audit RTM', miriboa: 'Bid document check', data: 'Data feeds', search: 'Search diagnostics', notes: 'Notes', work: 'Work', about: 'About', contact: 'Contact', founder: 'Founder' },
