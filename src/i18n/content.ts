  ...
  ko: {
    ...
    services: {
      ...
      rtm: {
        ...
        faq: [
          ...
          { q: '결과가 틀리면 어떻게 되나요?', a: '판정마다 원문 인용이 붙어 있어 PM 이 맞는지 바로 확인할 수 있습니다. 사람이 중간에 판정을 고치지 않으므로 틀린 판정은 인용을 보고 걸러 주셔야 하고, 표결이 갈리거나 인용이 원문에서 확인되지 않는 항목은 판정하지 않고 "확인 필요"로 둡니다. 감리 산출물 기준 정확도 수치는 아직 없고, 첫 프로젝트에서 골든셋을 만들어 같은 방식으로 공개합니다.' },
          ...
          { q: '왜 이 가격인가요?', a: '대조할 요구사항 수로 정합니다. 2026년 1~5월 나라장터 제안요청서 중 예산 5억~20억 개발형 사업 87건의 요구사항 고유번호는 중앙값 40건이라, 절반이 가장 낮은 구간에 듭니다. 같은 규모 사업의 감리 계약 39건(나라장터 계약정보, 2026년 9월 수집)은 감리 계약금액 중앙값이 약 7,200만원(설계·종료 두 단계 합계)이고, 120~300만원은 그 2~4% 수준입니다. 아직 감리 산출물 기준 정확도 측정치가 없어 첫 5건은 도입가로 받습니다. 종료 감리를 통과하지 못해 검수와 잔금이 한 달 밀리는 비용과 비교하시면 됩니다. 범위 확정 뒤 고정가라 추가 청구가 없습니다.' },
          { q: '자료는 어떻게 다루나요?', a: 'NDA 체결 뒤 파일로 받고, 납품 후 15일 안에 파기합니다. 외부 인력 없이 대표가 직접 수행합니다. 설계 반영 판정에서는 요구사항마다 잘라 낸 설계 절만 언어 모델(Anthropic Claude, 학습 거부 설정)로 보내고, 문서 전체는 보내지 않습니다. 다른 고객의 문서는 어떤 결과물에도 쓰이지 않습니다.' },
        ],
        nextH,
        next: [
    ...
    engine: {
      ...
      services: [
        ...
        { name: '감리 대비표 사전 검토', href: '/services/rtm/', what: 'RFP 요구사항을 설계 산출물과 대조하고, 대비표의 행과 ID 를 규칙으로 맞춥니다.', measured: '감리 산출물 기준 측정치 없음 — 첫 프로젝트에서 골든셋을 만들어 같은 방식으로 공개합니다.' },
