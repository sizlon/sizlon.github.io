  ...
  ko: {
    ...
    services: {
      ...
      rtm: {
        ...
        priceNote: '요구사항 고유번호 40건 이하 120만원 · 41~80건 200만원 · 81~150건 300만원. 부가세 별도, 보완본 재대조 1회 포함, 추가 청구 없음. 요구사항 수는 제안요청서의 요구사항 고유번호(SFR-001 같은)를 기계로 센 값이고, 151건 이상은 받지 않습니다. 설계단계 감리가 끝나고 30일 안에 감리 지적 결과(요구사항 ID·지적 여부·지적 유형만, 원문은 필요 없습니다)를 보내 주시면 40·70·100만원을 돌려드립니다. 외부 언어 모델 사용이 곤란하면 설계 반영 판정을 빼고 형식·추적성만 50·80·120만원에 봅니다(환급 없음). 사업비 20억 미만이거나 기간 6개월 미만인 사업은 감리기준상 2단계 감리(설계·종료)라 대개 설계단계 1회에 해당합니다. 20억 이상 3단계 감리의 요구정의 단계(과업대비표·요구사항정의서)는 맡지 않습니다.',
        ...
        how: [
          ...
          { name: '대조 — 세 겹', text: '감리 수행 가이드가 정한 감리원의 대비표 점검 세 겹을 문서만으로 먼저 봅니다.',
            items: [
              '형식 요건 — 회차에 맞는 열이 행마다 채워졌는가.',
              '추적성 — RFP 요구사항 고유번호가 빠짐없이 표에 있고, 표가 가리킨 화면·프로그램 ID 가 산출물 문서에 나오는가.',
        ...
        principles: [
          ...
          '사람이 판정을 고치지 않습니다. 결과물은 엔진 출력 그대로이고, 표결이 갈리거나 인용이 원문에서 확인되지 않는 항목은 "확인 필요"로 남깁니다.',
          '요구사항 반영과 추적성만 봅니다. 화면·프로그램·테이블·인터페이스 설계서 사이의 일관성, 그리고 사업관리·데이터베이스·시스템 구조·보안 같은 다른 감리 영역은 범위 밖입니다.',
