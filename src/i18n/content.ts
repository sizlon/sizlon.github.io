...
const proof = {
  ...
  // 요구사항 반영 사전 검토를 사람이 직접 하면(2026-09-15 오너 결정: '추정'이라 밝히고 사이트에 쓴다). 사람 작업 시간 실측 아님.
...
// 09-14: '사람이 승인합니다' 삭제 — 요구사항 반영 검토는 사람 검수 없음, 미리보아는 자동 납품 스위치에 따라 다르다(스위치 양쪽에서 참인 문장만).
...
export const content = {
  ko: {
    ...
    rtmPage: {
      ...
      // 09-17 오너: 설계 반영을 앞세움(감리 지적 1순위·DIY LLM 기준선에서 추적표·ID 대조는 PM 도 할 수 있음). 비교 우위 주장은 넣지 않는다 — 측정치 없음.
      sub: '설계단계 감리 통보를 받은 공공 SI 수행사 PM 을 위한 사전 검토입니다. 요구사항 대조 엔진이 요구사항마다 조건(수치·기한·권한·예외)이 설계서에 실제로 들어갔는지 원문 인용으로 대조하고, 추적표에 없는 요구사항과 설계서에 없는 ID 는 규칙으로 함께 찾습니다. 확정은 감리원이 합니다.',
      facts: [
      ...
      findings: [
        ...
        { code: 'C · 가리킨 ID 부재', kind: '규칙', head: '대비표가 가리킨 산출물 항목의 추적 가능 여부를 점검한 결과, 존재하지 않는 항목이 확인되므로 보완할 필요가 있음.', status: 'PGM-118 1건이 산출물에 없음(검색 문서 4건). PGM-118 → 추정: 이름이 같은 PGM-018「연계 이력 관리」(프로그램명세서)이 있음 — 표의 오기일 수 있음.', dir: '실제 산출물 ID 로 정정하거나 해당 산출물을 작성.' },
      ...
      checks: [
        ['형식 요건', '설계 산출물 열(화면·컴포넌트·프로그램·인터페이스·테이블)이 행마다 채워졌는지 — 유스케이스만 있으면 미기재 · 같은 요구사항의 중복 행 · 여러 요구사항에 걸린 ID', '규칙', '감리기준 제9조 · 감리 수행 가이드 2022 설계단계'],
        ['추적성', 'RFP 요구사항 고유번호 중 추적표에 없는 것 · 표가 가리킨 화면·프로그램·테이블 ID 가 산출물에 없는 것(이름이 같거나 표기만 다른 ID 가 있으면 원인 추정을 덧붙임) · 요구정의 단계 대비표에 있던 요구사항이 빠진 것(주실 때)', '규칙', '감리 수행 가이드 2022 · NCS IT감리 설계단계 2.2'],
      ...
      outs: [
        ...
        { name: '추적표 검토본', line: '보내 주신 추적표 xlsx 위에 판정·근거·비고 세 열을 붙입니다. 행 순서와 서식은 그대로입니다.' },
      ...
      dont: [
        '적합·부적합 확정 — 감리원이 합니다',
        '데이터베이스 밖 설계서 사이의 내용 일관성(예: 화면정의서와 프로그램명세서의 서술이 서로 다름) — 데이터베이스 설계 산출물끼리의 정합은 봅니다',
        '설계 품질 자체, 사업관리·시스템 구조·보안 영역',
        'ERD 가 그림으로만 있을 때의 대조, 실제 운영 DB 와의 대조',
        '요구정의 단계와 종료 단계 감리',
        '대조 범위(보통 기능 요구사항) 밖 요구사항이 거는 조건 — 예: 성능 요구사항의 목표치',
        '추적표 새로 만들기, 빈 칸 채우기, 산출물 대신 써 주기',
        '사람의 검수·정정 — 결과물은 엔진 출력 그대로입니다',
        '정확도 보장 — 감리 산출물 기준 정확도는 공유해 주신 감리 결과로 재고 있으며, 3개 이상 사업이 모이면 합산해 공개합니다',
      ],

      priceH: '가격',
      priceLead: '요구사항 고유번호 수로 정합니다. 데이터베이스 정합 검사는 추가 요금 없이 포함됩니다.',
      ...
      data: [
        ...
        '설계 반영 판정에서는 요구사항마다 잘라 낸 설계 산출물 일부만 언어 모델(Anthropic Claude, 학습 거부 설정)로 보냅니다. 문서 전체는 보내지 않습니다. 외부 언어 모델이 곤란한 사업이면 규칙 검사만 하고 가격표의 형식·추적성·DB만 값을 적용합니다.',
        '납품 후 15일 안에 파기합니다. 환급 대조를 위해 요구사항 ID 와 판정만 담은 표(원문·인용 없음)는 대조가 끝날 때까지 남기고, 대조가 끝나면 파기합니다.',
        '원문·사업명·식별자 값을 담지 않은 수량·양식·처리 통계(예: 요구사항 수, 표 머리글과 식별자의 형태, 유형별 결함 수)는 서비스 개선에 쓰고, 3개 이상 사업을 합산해서만 공개합니다. 원치 않으면 계약 때 거부할 수 있고, 거부해도 범위와 가격은 같습니다.',
        '따로 동의하시는 경우에만, 판정이 부적합·확인 필요로 나온 항목의 요구사항 원문과 판정에 쓰인 설계서 절(문서 전체가 아님)을 사업명·기관명을 가린 채 3년간 시험 사례로 보관해 판정 정확도를 개선합니다. 동의하지 않아도 범위와 가격은 같습니다.',
        '결과물마다 어느 엔진 판으로 만들었는지 기록해, 나중에 같은 판정을 설명할 수 있게 합니다.',
      ],

      faqH: '자주 묻는 것',
      faq: [
        { q: 'ChatGPT 같은 AI 에 파일을 넣으면 되지 않나요?', a: '추적표에 없는 요구사항이나 설계서에 없는 ID 를 찾는 일은 사업 규모가 작으면 그렇게도 됩니다. 차이는 요구사항의 조건이 설계에 실제로 들어갔는지를 보는 방식과, 판정마다 근거를 확인할 수 있게 남기는 데 있습니다. 엔진은 요구사항마다 추적표가 가리킨 ID 를 정의한 부분만 잘라 대조하고, 판정에 붙인 인용이 원문에 없으면 그 판정을 버리며, 표결이 갈리면 단정하지 않고 "확인 필요"로 둡니다. 규칙으로 닫히는 검사는 같은 파일이면 매번 같은 결과가 나오고, 결과물은 감리 개선권고 틀과 보내 주신 추적표 위 검토본으로 나옵니다.' },
    ...
    home: {
      ...
      description: '요구사항 대조 엔진과 데이터 파이프라인을 만드는 소프트웨어 회사. 요구사항 반영 검토·입찰 서류 검증·데이터 피드·검색 진단.',
    ...
    services: {
      ...
      rtm: {
        ...
        deliverables: [
          ...
          '추적표 검토본 — 고객의 요구사항추적표에 판정·근거·비고 세 열을 붙인 xlsx. 추적표를 xlsx 로 주시면 그 파일 위에 붙여 행 순서와 서식이 그대로입니다. 예: "SFR-020 | 화면 SCR-045 | 프로그램 PGM-118 → 부적합 예상 → 표가 가리킨 PGM-118 이 프로그램명세서에 없음"',
        ...
        scopeDont: {
          ...
          items: [
            ...
            '데이터베이스 밖 설계서 사이의 내용 일관성, 설계 품질 자체, 다른 감리 영역(사업관리·시스템 구조·보안)',   // 09-17 이 키는 페이지에 안 그려진다(RtmPage) — 문구만 맞춤
            '요구정의 단계와 종료 단계 감리',
            '추적표 새로 만들기, 빈 칸 채우기, 산출물 대신 써 주기',
            '사람의 검수·정정 — 결과물은 엔진 출력 그대로입니다',
        ...
        priceTable: {
          ...
          head: ['요구사항 고유번호 수', '가격', '감리 결과 공유 시 환급', '환급 뒤 실부담', '형식·추적성·DB만'],
          rows: [
            ['40건 이하', '120만원', '40만원', '80만원', '70만원'],
            ['41~80건', '200만원', '70만원', '130만원', '120만원'],
            ['81~150건', '300만원', '100만원', '200만원', '180만원'],
            ['151건 이상', '받지 않습니다', '—', '—', '—'],
          ],
          notes: [
            ...
            '형식·추적성·DB만: 외부 언어 모델 사용이 곤란할 때 설계 반영 판정만 빼고 규칙 검사(형식 요건·추적성·DB 정합)를 합니다. 환급은 없습니다.',
      ...
      data: {
        ...
        optionH: '',
        options: [],
        // 가진 문서 일괄 변환(2026-09-22 오너): 새 서비스가 아니라 이 피드의 추출 층을 고객 문서에 쓰는 일회성 고정가 납품.
        // 가격·상한은 크몽 상품과 같은 값(크몽은 부가세 포함 3.3만/22만/88만) — 정본 docs/articles/kmong-doc-conversion-listing-draft.md,
        // 집계·변환·삭제 도구 docs/tools/doc-conversion, 내보내기 dpurix structured/export.py (09-22 세 구간 모두 납품 코드 있음).
        priceTableH: '가진 문서를 한 번에 변환해 드립니다',
        priceTable: {
          caption: 'HWP·HWPX 문서 일괄 변환 · 고정가 · 부가세 별도',
          head: ['분량 (먼저 닿는 쪽)', '가격', '결과물', '작업일'],
          rows: [
            ['10건 또는 15만 자', '3만원', 'Markdown · 구조 JSON', '1일'],
            ['200건 또는 300만 자', '20만원', '+ 표마다 CSV · 엑셀 · 변환 보고서', '3일'],
            ['2,000건 또는 3,000만 자', '80만원', '+ RAG 적재용 JSONL', '7일'],
            ['그 이상 · 고객 DB 적재', '견적', '—', '—'],
          ],
          notes: [
            '표를 행·열·병합 셀까지 그대로 옮깁니다. 병합 없는 표는 Markdown 표로, 병합 있는 표는 병합 정보째 넣습니다. 구조 JSON 에는 칸마다 행·열·병합 범위가 들어갑니다.',
            '글자 수는 공백 제외, 시즐론 집계 기준입니다. 파일을 보내 주시면 주문 전에 무료로 건수·글자 수·표 수를 세어 맞는 구간을 알려 드립니다.',
            '받는 형식은 HWP·HWPX 입니다. PDF 는 원본 HWP 가 없을 때만 받습니다 — 같은 문서라도 PDF 는 표의 행 구조가 약 3분의 2만 남습니다. 스캔본·암호 문서는 받지 않습니다.',
            '규칙 기반 추출이라 문서를 외부 AI 서비스로 보내지 않습니다. 원본과 결과물은 납품 7일 뒤 삭제합니다.',
            '셀을 사람이 고쳐 적지 않습니다. 변환 결함이 나오면 추출기를 고쳐 다시 돌려 드립니다(무료).',
            '엑셀은 표마다 시트 하나에 병합을 그대로 두고, CSV 는 병합 칸을 비우거나 값으로 채워 드립니다(필터·집계용). RAG 적재용 JSONL 은 제목 경로를 단 청크이고, 표는 쪼개지 않아 항목과 숫자가 갈라지지 않습니다.',
            '공고문에서 마감일·예산처럼 특정 칸만 뽑는 일은 이 상품이 아닙니다 — 표본 10~20건으로 칸별 정확도를 먼저 보여 드리고 견적을 드립니다.',
          ],
        },
        ...
        minLine: '일회성 수집(크롤링) 50만원 미만은 받지 않습니다. 가진 문서 변환은 위 고정가로 받습니다.',
        faqH: '자주 묻는 것',
        faq: [
        ...
        next: [
          ...
          { label: '이 데이터가 공공 SI 산출물에 들어가면', line: '감리 전 요구사항 반영 검토 — 행마다 판정과 원문 근거까지.', href: '/services/rtm/' },
        ],
    ...
    engine: {
      ...
      description: '요구 문서의 요구사항을 제출 문서와 항목마다 원문 인용으로 대조하는 엔진.',
      eyebrow: '회사 · 엔진',
      h1: '시즐론의 요구사항 대조 엔진',
      lede: '요구 문서에서 요구조건을 원자 단위로 뽑고, 응답 문서가 그것을 채웠는지 원문 인용으로 대조합니다. 미리보아와 요구사항 반영 사전 검토가 이 엔진 위에 있고, 데이터 피드는 HWP·PDF 표 추출 층만 같이 씁니다(수집은 자체 크롤러와 API 로 따로, 판정 층은 쓰지 않습니다).',
      ...
      steps: [
        ...
        { name: '검증·납품', text: '판정이 나가기 전에 규칙 게이트를 다시 통과합니다. 인용이 원문에 실재하는가, 요약의 숫자가 본문과 같은가, 본 서류가 접수 서류와 같은가, 내부 흔적이 없는가. 원문에서 확인되지 않는 판정은 확정하지 않고 "확인 필요"로 둡니다.', measure: '입찰 서류 검증: 자동 게이트 7종 — 걸리면 운영자에게 넘어가고, 통과 건은 운영 설정에 따라 사람 검수를 거치거나 바로 납품됩니다. 요구사항 반영 검토: 사람 검수 없이 엔진 출력 그대로. 리포트 렌더러에는 언어 모델이 없어 같은 판정 자료에서 언제나 같은 리포트가 나옵니다.' },
      ],
      trustH: '판정을 어떻게 믿나',
      trust: [
        // 09-14: '독립 3표' 는 과장 — 같은 모델·같은 입력을 따로 세 번 호출(verify_pack.VOTES, rtm_sections 는 갈리면 +2표·5중 4). 오류가 연관돼 있다는 한계까지 적는다.
        '해석이 필요한 판정은 같은 언어 모델에 같은 질문과 문서를 서로 모르게 세 번 따로 묻습니다. 세 답이 모두 같고 근거 인용이 원문에 글자 그대로 있을 때만 "확실"로 내고, 아니면 "확인 필요"로 넘깁니다. 이 방식은 답이 흔들리는 판정을 걸러내지만, 모델이 세 번 똑같이 틀리는 경우까지는 막지 못합니다. 그래서 모든 판정에 원문 인용을 붙입니다. 요구사항 반영 검토는 세 답이 갈리면 두 번 더 물어 다섯 중 넷이 같을 때 그 판정을 씁니다. 모르면 모른다고 씁니다.',
      ...
      services: [
        ...
        { name: '요구사항 반영 사전 검토', href: '/services/rtm/', what: 'RFP 요구사항을 설계 산출물과 대조하고, 대비표의 행과 ID 를 규칙으로 맞춥니다.', measured: '감리 산출물 기준 측정치 없음 — 고객이 공유한 실제 감리 지적과 지적 예상 목록을 요구사항 ID로 대조한 수치를 쌓아 공개합니다.' },
    ...
    about: {
      ...
      lede: '시즐론은 서울 용산의 1인 소프트웨어 회사입니다. 심사받기 전에 지적당할 곳을 먼저 찾는 일 — 요구사항 반영 사전 검토, 입찰 서류 검증(미리보아) — 과 데이터 피드, 검색 품질 진단을 합니다. 판정은 시즐론이 만든 요구사항 대조 엔진이 하고, 그 엔진과 수집 시스템을 만든 대표가 직접 운영합니다.',
      sections: [
        { h: '무엇을 하는가 (시즐론, 2026.4~)', p: ['요구사항 반영 사전 검토, 입찰 서류 검증(미리보아), 데이터 피드, 검색 품질 진단(Elasticsearch·OpenSearch)을 합니다. 앞의 둘은 같은 요구사항 대조 엔진 위에 있고, 데이터 피드는 그 엔진의 표 추출 층만 같이 쓰며 수집은 자체 크롤러와 API 로 합니다. 검색 진단은 그 엔진을 만든 경력의 일입니다. 사이트가 바뀌면 스스로 복구하는 크롤러를 시즐론에서 만들었습니다.'] },
        { h: '작업 방식', p: [method] },
        // 고객의 걱정 셋(일정·결과물·자료)에 직접 답한다(2026-09-08). 새 보장 조건은 넣지 않는다.
        { h: '1인이라서 걱정되는 것', p: [
          '일정 — 동시에 받는 진단은 1건이고 기간은 고정입니다. 다른 일 때문에 밀리지 않습니다.',
          '결과물 — 요구사항 반영 검토와 검색 진단의 산출물은 파일과 절차서라 고객 손에 남습니다. 미리보아는 판정을 엔진이 사람 손 없이 하고, 데이터 피드는 자동으로 수집·복구되고 갱신 로그가 남습니다.',
      ...
      sections: [
        {
          ...
          p: [],
          list: [
            '시즐론 대표 (2026~) — 요구사항 반영 사전 검토 · 입찰 서류 검증(미리보아) · 데이터 피드 · 검색 품질 진단. 미리보아와 나라장터 개찰 데이터 파이프라인을 만들고 운영',
    ...
    contact: {
      ...
      topics: { rtm: '감리를 앞두고 있음 — 요구사항 반영 사전 검토', pilot: '입찰 마감을 앞두고 있음 — 미리보아 문의', datafeed: '데이터가 매달 필요함 — 데이터 피드', search: '검색이 못 찾음 — 검색 품질 진단', other: '그 밖의 문서 검증 · 기타' },
    ...
    legal: {
      ...
      terms: {
        ...
        sections: [
          ...
          { h: '서비스와 제품의 제공 방식', p: ['시즐론의 서비스 — 검색 품질 진단, 요구사항 반영 사전 검토, 데이터 피드 —는 통화로 범위를 정한 뒤 견적서와 개별 계약(NDA 포함)에 따라 수행합니다. 본 사이트의 가격은 범위 확정 전 기준가이며, 견적서가 우선합니다. 데이터 피드의 수집 원칙은 /services/data#principles 에 게시하며 견적서·계약서가 이를 참조합니다.', '시즐론이 소프트웨어를 납품하는 경우, 그 소프트웨어는 판매가 아니라 라이선스로 제공되며, 오프라인으로 검증되는 벤더 발급 라이선스로 납품됩니다. 온라인 서비스 — 미리보아 —는 Sizlon이 자체 주소에서 운영하며, 해당 사이트에서 계정을 만들고 그 사이트에 게시된 약관에 따라 이용하십니다. 어느 경우든 본 웹사이트 내용과 상충하면 계약, 라이선스 또는 해당 서비스 약관이 우선합니다.'] },
      ...
      privacy: {
        ...
        sections: [
          ...
          { h: '서비스·제품 데이터', p: ['서비스 수행 중 고객이 제공하는 자료(로그·설정·RFP·산출물·수집 대상 목록 등)는 요청하신 업무에만 사용하고, 아래 처리 위탁을 제외하면 제3자에게 제공하지 않으며, 계약이 정한 기간(기본 15일) 안에 파기합니다. 데이터 피드로 수집한 데이터는 해당 고객에게만 납품하고 재판매·재사용하지 않습니다.', '요구사항 반영 사전 검토에는 네 가지 예외가 있습니다. 첫째, 설계 반영 판정을 위해 요구사항마다 잘라 낸 설계 산출물의 일부(문서 전체가 아님)를 언어 모델 제공자 Anthropic, PBC(미국)에 보내 처리합니다. 모델 학습에 쓰지 않도록 설정한 계정을 쓰며, 제공자 쪽 보관 기간은 최대 30일이고 안전 검토 목적의 예외가 있습니다. 외부 언어 모델 사용을 원하지 않으시면 이 판정을 빼고 형식 요건·추적성·데이터베이스 정합만 봅니다. 둘째, 감리 결과 공유에 따른 환급 대조를 위해 요구사항 ID와 판정만 담은 표(원문·인용 없음)를 대조가 끝날 때까지 보관하고, 대조가 끝나면 파기합니다. 셋째, 원문·인용·요약, 사업명·기관명·고객사명, 요구사항 ID 등 식별자의 값, 개인정보를 담지 않은 운영 정보 — 요구사항·산출물·테이블의 수, 표 머리글 이름과 식별자의 형태, 검사 유형별 결함 수와 처리 시간, 감리 결과와의 유형별 일치 수 — 는 서비스 개선과 통계 작성을 위해 보관하며, 공개할 때는 3개 이상 사업을 합산한 형태로만 공개합니다. 고객은 계약 때 이 보관을 거부할 수 있고, 거부해도 범위와 가격은 같습니다. 넷째, 고객이 별도로 동의한 경우에만, 판정이 부적합·확인 필요로 나온 항목의 요구사항 원문·추적표 해당 행·판정에 쓰인 설계서 절을 사업명·기관명·고객사명을 가린 채 판정 정확도 개선용 시험 사례로 3년간 보관하며, 기간이 지나거나 동의를 철회하면 파기합니다.', '라이선스된 Sizlon 소프트웨어는 귀하의 통제 하에 온프레미스 또는 연결형으로 배포됩니다 — 추출·처리되는 데이터는 귀하 환경에 머물며, Sizlon은 지원 업무상 명시적으로 필요하고 귀하가 승인한 경우를 제외하고 접근하지 않습니다.', '미리보아처럼 Sizlon이 운영하는 온라인 서비스의 경우, 보내주신 서류는 요청하신 검증 수행에만 사용하며, 처리 위탁·국외 이전과 보관·삭제는 해당 서비스의 개인정보처리방침이 정하는 바에 따릅니다.'] },
          { h: '처리 위탁 및 국외 이전', p: ['사이트 운영, 문의 처리, 서비스 수행을 위해 소수의 서비스 제공자를 이용하며, 이들의 서버는 대한민국 외(주로 미국)에 위치할 수 있습니다. GitHub, Inc. — 웹사이트 호스팅(GitHub Pages, 표준 서버 로그). 문의 양식 접수 내용과 방문 통계는 Sizlon 자체 서버에 보관됩니다. Google LLC — 신규 문의 알림 메일 전달(Gmail). Cloudflare, Inc. — Turnstile 스팸 방지. Anthropic, PBC(미국) — 요구사항 반영 사전 검토의 설계 반영 판정(요구사항마다 잘라 낸 설계 산출물 일부, 인터넷 전송, 제공자 보관 최대 30일). 산출물에 개인정보가 들어 있는 경우 그 부분이 함께 전송될 수 있습니다.', '각 제공자는 해당 서비스 제공 목적으로만 개인정보를 처리하며 자체 개인정보 보호 약정의 적용을 받습니다. 이들 외의 제3자에게는 법률상 요구되는 경우를 제외하고 개인정보를 제공하지 않습니다.'] },
