    ...
    services: {
      ...
      rtm: {
        ...
        who: [
          ...
          '설계 열까지 채운 요구사항추적표가 있고, 감리 전에 그 표가 설계 산출물과 실제로 맞는지 확인하고 싶은 팀. 지난 감리에서 요구사항 반영 미흡이나 추적표 ID 불일치로 개선권고를 받은 적이 있는 팀. 빠진 항목은 그대로 지적사항이 되고, 고치는 비용은 수행사 몫입니다.',
        ],
        howH: '진행',
        how: [
          { name: '자료 접수 — 당일', text: '꼭 필요한 것은 셋입니다. ① RFP(제안요청서) — 요구사항 고유번호 표가 들어 있는 원본 파일. ② 요구사항추적표 — 요구사항 ID 가 RFP 고유번호이거나 그 번호로 시작해야 하고(SFR-001, SFR-001-01), 자체 번호를 쓰면 RFP 고유번호를 적은 출처 열이 있어야 합니다. xlsx 로 주시면 그 파일 위에 결과를 붙입니다. ③ 추적표가 가리키는 ID(화면·프로그램 등)가 들어 있는 설계서 전부. 있으면 검사기준서도 함께 봅니다. NDA 뒤 파일로 받고, 다음 날 안에 범위와 고정가를 회신합니다.' },
          { name: '대조·보고 — 영업일 5일 안', text: '감리원이 대비표에서 보는 세 겹(형식 요건·추적성·설계 반영)을 감리 전에 먼저 보고, 아래 결과물 셋을 드립니다. 엔진 출력 그대로이며 사람이 판정을 고치지 않습니다. 보완본을 주시면 재대조 1회는 가격에 포함됩니다.' },
        ],
        ...
        effortLead: '감리원이 대비표에서 보는 세 겹(형식 요건·추적성·설계 반영)을 사람이 직접 하면, 설계를 마무리하는 기간에 한 사람을 1~1.5주 따로 떼어야 합니다. 맡기시면 PM 이 하는 일은 자료를 보내고 결과의 원문 인용을 확인하는 것입니다.',
        effortCards: [
          { label: '직접 대조 · 추정', big: '4.5~7.5일', cost: '인건비 약 190만~310만원', sub: '한 사람이 설계 마무리와 따로' },
        ...
        scopeDo: {
          ...
          groups: [
            ...
            { label: '언어 모델 — 예상', items: [
              '설계 반영: 표가 가리킨 화면·프로그램 ID 를 정의한 부분이 요구사항의 핵심 조건(수치·기한·권한·예외)을 담는지. 목차·목록에 ID 가 나온 자리보다 정의한 자리를 먼저 봅니다. 원문 인용을 붙여 적합 예상 · 부적합 예상 · 확인 필요로 표시합니다.',
            ] },
          ],
        },
        scopeDont: {
          ...
          items: [
            ...
            '요구정의 단계와 종료 단계 감리',
            '추적표 새로 만들기, 빈 칸 채우기, 산출물 대신 써 주기 — 설계 열이 빈 행은 형식 요건 미기재로 표시하고, 요구사항과 용어가 겹치는 설계 부분으로 판정한 결과만 드립니다',
            '사람의 검수·정정 — 결과물은 엔진 출력 그대로이고, 판정이 갈린 항목은 "확인 필요"로 남습니다',
        ...
        priceTable: {
          ...
          notes: [
            '요구사항 수는 제안요청서의 요구사항 고유번호(SFR-001 같은)를 기계로 센 값입니다. 자료를 받은 다음 날 안에 구간과 금액을 회신합니다.',
        ...
        faqH: '자주 묻는 것',
        faq: [
          { q: '요구사항추적표가 아직 없는데 맡길 수 있나요?', a: '추적표를 만들어 드리지는 않습니다. 대비표는 감리기준상 수행사가 설계단계 감리 전까지 발주자 확인을 거쳐 내야 하는 문서라, 표가 나온 뒤에 맡기시면 그 표 위에 판정을 붙여 드립니다.' },
          { q: 'PMO 가 이미 추적표를 검토하는데요?', a: '표가 채워졌는지에 더해, 표가 가리킨 화면·프로그램 ID 의 설계 내용을 열어 요구사항의 핵심 조건이 들어 있는지까지 봅니다. 2012년 감리보고서 169건 분석에서 지적 1순위가 "요구분석과 설계의 일관성"이었습니다.' },
          ...
          { q: '결과가 틀리면 어떻게 되나요?', a: '판정마다 원문 인용이 붙어 있어 PM 이 바로 확인할 수 있습니다. 사람이 중간에 판정을 고치지 않으므로 틀린 판정은 인용을 보고 걸러 주셔야 하고, 표결이 갈리거나 인용이 원문에서 확인되지 않는 항목은 "확인 필요"로 둡니다. 감리 산출물 기준 정확도 측정치는 아직 없어, 감리 뒤 지적 결과를 보내 주시면 대조해 수치를 쌓고 일부를 돌려드립니다.' },
          ...
          { q: '왜 이 가격인가요?', a: '대조할 요구사항 수로 정합니다. 개발형 사업 87건의 요구사항 고유번호 중앙값이 40건이라 절반이 가장 낮은 구간에 듭니다. 같은 일을 직접 하면 드는 시간과 인건비 추정은 위 "직접 대조하면 얼마나 걸리나"에 있습니다. 같은 규모 사업의 감리 계약금액 중앙값(약 7,200만원, 39건)에 비하면 2~4% 수준입니다.' },
          { q: '자료는 어떻게 다루나요?', a: 'NDA 체결 뒤 파일로 받고, 납품 후 15일 안에 파기합니다. 환급 대조를 위해 요구사항 ID와 판정만 담은 표(원문·인용 없음)는 대조가 끝날 때까지 남깁니다. 외부 인력 없이 대표가 직접 수행합니다. 설계 반영 판정에서는 요구사항마다 잘라 낸 설계 산출물 일부만 언어 모델(Anthropic Claude, 학습 거부 설정)로 보내고, 문서 전체는 보내지 않습니다. 다른 고객의 문서는 어떤 결과물에도 쓰이지 않습니다.' },
        ],
        nextH,
        next: [
