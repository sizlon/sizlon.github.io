...
export const content = {
  ko: {
    ...
    rtmPage: {
      ...
      faqH: '자주 묻는 것',
      faq: [
        { q: 'ChatGPT 같은 AI 에 파일을 넣으면 되지 않나요?', a: '추적표에 없는 요구사항이나 설계서에 없는 ID 를 찾는 일은 사업 규모가 작으면 그렇게도 됩니다. 차이는 요구사항의 조건이 설계에 실제로 들어갔는지를 보는 방식과, 결과를 믿고 쓸 수 있느냐에 있습니다. 엔진은 요구사항마다 추적표가 가리킨 ID 를 정의한 부분만 잘라 대조하고, 판정에 붙인 인용이 원문에 없으면 그 판정을 버리며, 표결이 갈리면 단정하지 않고 "확인 필요"로 둡니다. 규칙으로 닫히는 검사는 같은 파일이면 매번 같은 결과가 나오고, 결과물은 감리 개선권고 틀과 보내 주신 추적표 위 검토본으로 나옵니다. 어느 쪽이 더 정확한지는 아직 재지 않았습니다.' },
