    ...
    services: {
      ...
      rtm: {
        ...
        how: [
          ...
          { name: '대조 — 세 겹', text: '감리 수행 가이드가 정한 감리원의 점검 순서를 그대로 먼저 합니다. ① 형식 요건: 대비표가 있고 회차에 맞는 열이 채워졌는가. ② 추적성: 요구사항이 빠짐없이 표에 있고, 표가 가리킨 화면·프로그램 ID 가 산출물에 실제로 있는가. ③ 설계 반영: 그 절이 요구사항의 핵심 조건(수치·기한·권한·예외)을 담고 있는가. ①②는 기계적 대조이고 ③은 항목마다 원문을 인용해 판정합니다.', note: '세 겹의 출처와 감리원이 실제로 지적하는 것:', link: { href: '/notes/audit-findings/', label: '감리 지적 통계와 점검 절차' } },
