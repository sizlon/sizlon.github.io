...
export const content = {
  ko: {
    ...
    home: {
      ...
      lede: '시즐론이 만든 문서 검증 엔진이 감리원·평가위원이 볼 요구 문서와 제출 서류를 항목마다 대조해, 빠진 것과 어긋난 것을 원문 근거와 함께 냅니다. 어떻게 재는지와 얼마나 틀리는지는 공개합니다. 엔진은 대표가 만들고 직접 운영합니다.',
      ...
      cards: [
        { key: 'rtm', situation: '감리 앞', name: '감리 대비표 사전 검토', terms: '설계단계 감리 1회 120~300만원(요구사항 수 구간별)', deliverable: '대비표 검토본(행마다 판정·근거) + 지적 예상 목록', relation: '공공 SI 수행사 PM. 설계 산출물이 나오고 감리 통보를 받은 시점에 맡깁니다. 대표가 직접 수행합니다.' },
        { key: 'miriboa', situation: '입찰 마감 앞', name: '입찰 서류 검증 — 미리보아', terms: '셀프서비스 · 원문 근거를 붙인 기계 판정', deliverable: '공고 요구조건마다 응답 여부 판정 + 근거 인용 리포트', relation: '나라장터·미 연방조달 응찰 업체. 마감 전에 공고와 서류를 올리면 항목별 판정이 돌아옵니다.' },
      ],
      // 심사 프레임 밖의 둘 — 서비스로는 유지하되 작은 카드로(v4 §1: 검색 진단은 이 엔진을 만든 경력의 일).
      othersEyebrow: '그 밖에 하는 일',
      othersH2: '심사 앞이 아니어도, 같은 손으로 합니다.',
      others: [
        { key: 'data', name: '데이터 피드', terms: '구축 100~300만원 + 월 30~80만원', deliverable: '매달 오는 정제 데이터', relation: '판정과 분석의 근거가 되는 공공 데이터가 필요할 때. 개찰 원자료를 매일 받는 파이프라인으로 합니다.' },
        { key: 'search', name: '검색 품질 진단', terms: '2주 · 250만원', deliverable: '진단 보고서 + 즉시 적용 설정', relation: '색인은 있는데 못 찾을 때. Elasticsearch·OpenSearch·Nori.' },
      ],
      ...
      methodEyebrow: '작업 방식',
      method,
      // "왜 이 사람인가" 절은 09-06 에 없앴다 — 경력은 대표 소개가(09-14 오너 결정: 히어로 부제에서 크롤링 경력 삭제 — 심사받는 쪽 구매자 질문과 축이 달라서), 실적 숫자는
    ...
    engine: {
      ...
      steps: [
        ...
        { name: '검증·납품', text: '판정이 나가기 전에 규칙 게이트를 다시 통과합니다. 인용이 원문에 실재하는가, 요약의 숫자가 본문과 같은가, 본 서류가 접수 서류와 같은가, 내부 흔적이 없는가. 걸리면 사람 검수로 갑니다.', measure: '자동 게이트 7종, 그 뒤 사람 검수 1회. 리포트 렌더러에는 언어 모델이 없어 같은 판정 자료에서 언제나 같은 리포트가 나옵니다.' },
      ],
      trustH: '판정을 어떻게 믿나',
      trust: [
        '해석이 필요한 판정은 독립 3표가 일치해야 "확실"이고, 아니면 "확인 필요"로 넘깁니다. 모르면 모른다고 씁니다.',
        '규칙으로 닫히는 것은 언어 모델 없이 잽니다. 언어 모델이 낸 판정도 규칙 게이트를 다시 통과해야 나갑니다.',
        '인용이 원문에 글자 그대로 있어도 뜻까지 맞는다는 보장은 아닙니다. 그래서 인용을 붙여 사람이 30초 안에 확인할 수 있게 둡니다.',
      ],
      ...
      servicesH: '이 엔진으로 하는 일 둘, 그리고 층 하나만 같이 쓰는 일',
      services: [
